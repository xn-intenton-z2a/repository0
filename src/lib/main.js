...
function loadConfig() {
  try {
    // Use direct fs methods so that tests spying on fs work as expected
...
const helpMessage =
  ...
  '  config <subcommand>    Manage CLI configuration (subcommands: view, set)\n' +
  '\n' +
    ...
    .command(
      ...
      (yargs) => {
        return yargs
          ...
          .demandCommand(1, 'Specify view or set as a subcommand for config');
      },
      () => {}
    )
    .help(false)
