// src/lib/cron.js
// Pure ESM cron parsing and scheduling utilities
...
export function toString(parsed) {
  ...
  const secs = parsed.hasSeconds ? fieldToStr(f.seconds,0,59) : null;
  const mins = fieldToStr(f.minutes,0,59);
  const hrs = fieldToStr(f.hours,0,23);
  const dom = fieldToStr(f.dayOfMonth,1,31);
  const mon = fieldToStr(f.month,1,12);
  const dow = fieldToStr(f.dayOfWeek,0,6);
...
export function matches(expression, date) {
  ...
  // Day-of-month and day-of-week follow cron semantics: if both are restricted, match when either field matches.
...
export function nextRun(expression, after) {
  ...
  limit.setFullYear(limit.getFullYear() + 5); // search up to 5 years
