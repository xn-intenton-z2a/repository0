...
Behaviour
- Compare corresponding code points with strict equality and return the number of differing positions as a non-negative Number.
- Empty strings are valid and equal; hammingDistance("", "") returns 0.
- Deterministic error keywords that tests match: string, options, normalize, length.
...
README and examples
- README.md must document the three named exports with signatures and succinct examples, including the normalize option.
