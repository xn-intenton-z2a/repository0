...
**Enhancement:** The conversation history export commands (chat-export, chat-html-export, chat-pdf-export, and now chat-csv-export) have been enhanced to include session metadata, individual message timestamps, and support filtering by a specified date range using the `--start-date` and `--end-date` options. In addition, they support a new `--template` option to allow users to provide a custom EJS template for export. When provided, the export file is generated by processing the given template with the conversation data.
...
- **chat-csv-export:** Exports the conversation history to a CSV file (`chat_history.csv`). The CSV includes session metadata (session title and export timestamp) in the header, and each row contains the index, role, content, timestamp, feedback (if provided), and tags. Supports filtering by tag and date range.
  - Example: `repository0 chat-csv-export --tag important --start-date 2023-05-01 --end-date 2023-06-01`
