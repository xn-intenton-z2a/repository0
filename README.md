...
**Enhancement:** The conversation history export commands (chat-export, chat-html-export, chat-pdf-export) have been enhanced to include session metadata, individual message timestamps, and now support filtering by a specified date range using the `--start-date` and `--end-date` options. In addition, they now support a new `--template` option to allow users to provide a custom EJS template for export. When provided, the export file is generated by processing the given template with the conversation data. 

A new **chat-feedback** command has been introduced to allow users to attach, remove, and view feedback on individual conversation entries. This functionality enables users to annotate messages (for example, with 'positive' or 'negative' feedback) to aid in conversation analysis.
...
- **chat-export:** Exports the conversation history to a markdown file (`chat_history.md`). Optional flags include:
    ...
    - `--template`: Provide a path to a custom EJS template for export. When provided, the conversation data and metadata (session title, export timestamp, date range, individual timestamps, and feedback if available) are injected into the template to generate the file.
  - Example: `repository0 chat-export --tag important --start-date 2023-05-01 --end-date 2023-06-01 --template ./custom_template.md`
- **chat-html-export:** Exports the conversation history to an HTML file (`chat_history.html`). Supports optional `--tag`, `--start-date`, `--end-date`, and `--template` flags for filtered and customized export. The HTML export includes a header with the session title, export timestamp, date range, and displays each message's timestamp (and feedback if available).
...
- **chat-pdf-export:** Exports the conversation history to a PDF file (`chat_history.pdf`). Supports optional `--tag`, `--start-date`, `--end-date`, and `--template` flags for filtered and customized export. The PDF export includes a header with the session title, export timestamp, date range, and each message's timestamp (and feedback if available) in the output. In the test environment, it outputs plain text for easier validation of metadata.
...
- **chat-feedback:** Manage feedback on conversation entries. Subcommands include:
  - `add`: Attach feedback to a conversation entry (e.g., `repository0 chat-feedback add --index 2 --feedback "positive"`).
  - `remove`: Remove feedback from a conversation entry (e.g., `repository0 chat-feedback remove --index 2`).
  - `list`: List feedback for all conversation entries.
...
The export commands (chat-export, chat-html-export, chat-pdf-export) have been enhanced to include session metadata, individual message timestamps, and now support filtering by a date range. Users can use the `--start-date` and `--end-date` options (in ISO format) to filter which conversation entries are exported. Additionally, a new `--template` option lets you supply an EJS template file to customize the export format. The rendered template is provided with an object containing the session title, export timestamp, date range, and an array of messages. The export files will display the session title, export timestamp, the applied date range, and each message's timestamp (and feedback if provided).
...
The **chat-feedback** command allows users to annotate individual conversation entries with feedback, which can later be reviewed or removed. This feature can help in analyzing conversation quality and user satisfaction.

The **chat-search** command has been enhanced to support an optional `--regex` flag. When this flag is provided, the query is interpreted as a regular expression (case-insensitive), allowing for more advanced and flexible searches. If the regex pattern is invalid, an error will be displayed.
...
- **Source Code:**
    The CLI functionality is implemented in `src/lib/main.js` with modular commands including enhanced export commands supporting custom templates, date range filters, auto archival, chat session title management, conversation restoration, and feedback management.
- **Global Verbose Mode:**
