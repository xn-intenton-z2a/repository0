...
## plot-code-lib CLI examples

Generate an SVG of y=sin(x):

```bash
node src/lib/main.js --expression "y=sin(x)" --range "x=-3.1416:3.1416" --points 200 --file examples/sin.svg
```

Generate a PNG (rasterised approximation):

```bash
node src/lib/main.js --expression "y=sin(x)" --range "x=-3.1416:3.1416" --points 400 --file examples/sin.png
```

The CLI accepts:
- --expression : expression like "y=sin(x)" or "y = x^2"
- --range : range in the form "x=min:max"
- --points : number of sample points
- --file : output path (svg or png)

Files written with --file are saved relative to the repository root. The SVG is vector output; the PNG is a simple raster approximation produced by the library.
